...
TEST_DF_PATH = "/usr/src/app/imbalance_price"
...
def prepare_saved_df():
    ...
    test_df.write_delta(TEST_DF_PATH)
    yield
...
def mock_fetch_semo(mocker):
    mock = mocker.patch('grid_data.extract_semo.fetch_semo_xml', return_value=SAMPLE_SEMO_XML)
    yield mock

def test_main_existing_data(mock_fetch_semo, prepare_saved_df):
    run_time = "2023-12-26T11:35:12"
    extract_semo.main(run_time=run_time, base_path=BASE_PATH)

    df = pl.read_delta(TEST_DF_PATH)
